...
access(all) contract FlowToken: FungibleToken {
    ...
    // WEIRD FLOWTOKEN INIT FUNCTION THAT I CAN'T SEEM TO MAKE WORK - THIS CONTRACT IS UNDEPLOYABLE
    ...
    init() {
        ...
        if (self.devMode) {
            let randomVault: @AnyResource <- self.account.load<@AnyResource>(from: /storage/flowTokenVault)
            destroy (randomVault)

            let randomAdmin: @AnyResource <- self.account.load<@AnyResource>(from: /storage/flowTokenAdmin)
            destroy (randomAdmin)

            self.account.unlink(/public/flowTokenReceiver)
            self.account.unlink(/public/flowTokenBalance)
        }
