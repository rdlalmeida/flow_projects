    ...
    prepare(signer: auth(Storage, Capabilities) &Account) {
        ...
        if (self.voteBoxRef ==  nil) {
            ...
            log(
                ...
                .concat(" already has a VoteBox in storage.")
            )
        }

        // Check if the capabilities are enabled as well
        ...
        var storageCapabilities: Capability<&VoteBooth_std.VoteBox>? = signer.capabilities.get<&VoteBooth_std.VoteBox>(VoteBooth_std.voteBoxPublicPath)

        if (storageCapabilities == nil) {
            // No public storage capabilities published yet. Deal with it
            signer.capabilities.publish(
                signer.capabilities.storage.issue<&VoteBooth_std.VoteBox>(VoteBooth_std.voteBoxStoragePath),
                at: VoteBooth_std.voteBoxPublicPath
            )
        }
    }
