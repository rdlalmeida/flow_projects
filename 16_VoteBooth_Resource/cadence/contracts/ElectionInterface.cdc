...
access(all) contract interface ElectionInterface {
    ...
    // TODO: Create a public interface to hide the contents from the Election in the main VoteBooth contract
    ...
    access(all) resource interface Election: Burner.Burnable {
        ...
        **/
        access(all) fun submitBallot(ballot: @{BallotInterface.Ballot}): Void {
            pre {
            ...
            **/
            if (randomResourceRef == nil) {
            ...
            else if (randomResourceRef.getType() == Type<&{BallotInterface.Ballot}?>()) {
                ...
                let oldBallotId: UInt64 = (randomResourceRef as! &{BallotInterface.Ballot}).ballotId

                // Replace the old Ballot with the new one
                let oldBallot: @{BallotInterface.Ballot}? <- self.storedBallots[newBallotIndex] <- ballot
        ...
        **/
        access(ElectionAdmin) fun withdrawBallots(): @[{BallotInterface.Ballot}] {
            ...
            var ballotsToTally: @[{BallotInterface.Ballot}] <- []
            ...
            for ballotIndex in ballotIndexes {
                let currentBallot: @{BallotInterface.Ballot} <- self.storedBallots.remove(key: ballotIndex)!
        ...
        access(contract) fun burnCallback(): Void {
            ...
            **/
            ...
            for storedBallotIndex in storedBallotsIndexes {
                let ballotToBurn: @{BallotInterface.Ballot}? <- self.storedBallots.remove(key: storedBallotIndex)
