...
command_string="flow emulator start --port=${PORT} --rest-port=${REST_PORT} --admin-port=${ADMIN_PORT} --verbose=${VERBOSE} --log-format=${LOG_FORMAT} --block-time=${BLOCK_TIME} --chain-id=${CHAIN_ID} --contract-removal=${CONTRACT_REMOVAL} --coverage-reporting=${COVERAGE_REPORT} --evm-enabled=${EVM_ENABLED} --contracts=${CONTRACTS} --service-sig-algo=${SERVICE_SIG_ALGO} --service-priv-key=${SERVICE_PRIV_KEY} --service-hash-algo=${SERVICE_HASH_ALGO} --init=${INIT} --rest-debug=${REST_DEBUG} --grpc-debug=${GRPC_DEBUG} --persist=${PERSIST} --dbpath=${DB_PATH} --simple-addresses=${SIMPLE_ADDRESSES} --token-supply=${TOKEN_SUPPLY} --transaction-expiry=${TRANSACTION_EXPIRY} --storage-limit=${STORAGE_LIMIT}"
